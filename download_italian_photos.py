...
def download_italian_photos():
    ...
    # Filter for Italian League players
    italian_players = [p for p in players if p.get('lig') == 'İtalya Ligi']
    
    print(f"Found {len(italian_players)} Italian League players.")
    
    count = 0
    errors = 0
    
    for i, player in enumerate(italian_players):
        ...
        print(f"[{i+1}/{len(italian_players)}] Downloading {player['isim']}...")
        ...
        try:
             # 1. Fetch profile page to get image URL (same logic as v2)
            ...
            if img_url and '/header/' in img_url:
                 # Check 'big' or 'medium'
                 # But to be fast, just use what we find or mapped 'medium' blindly if we trust it exists?
                 # Let's request 'big' first

...
def download_images():
    ...
    try:
        if os.path.exists(file_path):
            ...
            # Filter for Manchester City players in English League
            city_players = [p for p in data if p.get('lig') == "İngiltere Ligi" and p.get('kulup') == "Manchester City"]
            print(f"Found {len(city_players)} Manchester City players. Starting downloads...")

            for i, player in enumerate(city_players):
                name = player['isim']
                url = player.get('url')
                ...
                print(f"[{i+1}/{len(city_players)}] Processing {name}...")
